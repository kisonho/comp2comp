...
class MuscleAdiposeTissueMetricsSaver(InferenceClass):
    ...
    def __call__(self, inference_pipeline, results):
        ...
        self.model_name = inference_pipeline.muscle_adipose_tissue_model_name
        self.output_dir = inference_pipeline.output_dir
        self.age = inference_pipeline.age
        self.csv_output_dir = os.path.join(self.output_dir, "metrics")
        self.segmentation_output_dir = os.path.join(self.output_dir, "segmentations")
    ...
    def save_results(self, results):
        ...
        pixel_spacing = image.header["pixdim"][1:4]
        image = image.get_fdata()
        seg = nib.load(seg_path)
        seg = nib.as_closest_canonical(seg)
        seg = seg.get_fdata()
        sat_seg = (seg == 1).astype(int)
        vat_seg = (seg == 2).astype(int)

        try:
            position_anterior_most_sat_pixel = np.max(np.where(sat_seg == 1)[1])
        except:
            print("No SAT")
            return
        air_seg = np.zeros_like(sat_seg)
        ...
        hu_vat = -95
        hu_air = -1000
        slope = (hu_vat - hu_air) / (vat_measured - air_measured)
